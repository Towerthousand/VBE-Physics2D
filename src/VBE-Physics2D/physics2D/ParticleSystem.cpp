...
namespace Physics {
	...
	void ParticleSystem::queryAABB(QueryCallback* callback, const AABB& aabb) const {
		b2AABB b2aabb;
		b2aabb.lowerBound = b2Vec2(aabb.getMin().x, aabb.getMin().y);
		b2aabb.upperBound = b2Vec2(aabb.getMax().x, aabb.getMax().y);
		queryCallback.reset(callback, this);
		sys->QueryAABB(&queryCallback, b2aabb);
	}

	bool ParticleSystem::SystemQueryCallback::ReportFixture(b2Fixture* fixture) {
	...
	bool ParticleSystem::SystemQueryCallback::ReportParticle(const b2ParticleSystem* particleSystem, int32 index) {
		(void) particleSystem;
		return callback->reportParticle(callbackSystem, index);
	}

	void ParticleSystem::SystemQueryCallback::reset(QueryCallback* newCallback, const ParticleSystem* system) {
		callback = newCallback;
		callbackSystem = const_cast<ParticleSystem*>(system);
	}
